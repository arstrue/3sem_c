...
template <>
std::ostream& operator<<(std::ostream& out, const std::pair<char, char>& p)
{
    out << '{';
    if (p.first == '\0')
        out << "'\\0'";
...
std::ostream& operator<<(std::ostream& out, const std::array<T, U>& a)
{
    out << '{';
...
template <int U>
std::ostream& operator<<(std::ostream& out, const std::array<std::string, U>& a)
{
    out << '{';
    for (size_t i = 0; i < U - 1; ++i)
        out << '"' << a[i] << '"' << ", ";
...
public:
    void allocate() { _ptr = (T*)std::malloc(sizeof(T)); }

    void construct(const T& t) { new (_ptr) T{ t }; }

    void destruct() { _ptr->~T(); }

    void deallocate() { free(_ptr); }
...
void test6()
{
    ...
    cout << v << " -> " << pairing(v) << endl;

    std::array<std::string, 4> a{ "cat", "dog", "mouse", "lion" };
    cout << a << " -> " << pairing(a) << endl;

    std::string_view s{ "Hello" };
    cout << '"' << s << '"' << " -> " << pairing(s) << endl << endl;
}
